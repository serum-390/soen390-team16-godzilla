  ...
  { field: 'productionButton', headerName: 'Production', width: 130, renderCell: params => (
    <div style={{ margin: 'auto' }}>
      {params.getValue("production") === 0
        ? <Button variant='contained' color='primary' onClick={(newSelection) => { ToggleProduction(params.getValue("id"), params.getValue("production")); }}>Start</Button>
        : <Button variant='contained' color='primary' onClick={(newSelection) => { ToggleProduction(params.getValue("id"), params.getValue("production")); }}>Stop</Button>
      }
    </div>
  )}
];
